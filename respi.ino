 ...
 * ///////////////////////////////////////////////////////////////////////////
 *
 * Uses code of Kris Winer for the MPU6050
 * date: May 1, 2014
 ...
 * parameterizing the register addresses. Added display functions to allow 
 * display to on breadboard monitor. 
 * No DMP use. We just want to get out the accelerations, temperature, and 
 * gyro readings.
 * 
 ...
 * Because the sensor is not 5V tolerant, we are using a 3.3 V 8 MHz Pro Mini
 * or a 3.3 V Teensy 3.1.
 * We have disabled the internal pull-ups used by the Wire library in the 
 * Wire.h/twi.c utility file.
 * We are also using the 400 kHz fast I2C mode by setting the TWI_FREQ to
 * 400000L /twi.h utility file.
*/
...
#include <Wire.h>   // Wire lirary is used control the diplay via I2C.
...
// Rev 4.2, 08/19/2013 6 DOF Motion sensor fusion device
...
// Address of the MPU5060; needed for the I2C communication.
...
// moving average buffer for accelerometer and gyroscope sensor values
#define BUF_SIZE 3
...
void setup (void) {
    ...
    if (!log_file.open("log.txt", O_WRITE | O_CREAT))
    sd.errorHalt_P(PSTR("Cannot create log.txt file"));

    cout << pstr("ok") << endl;

    cout << pstr("Start the MPU6050...");

    // Start device display with ID of sensor.
    tft.fillScreen(ILI9341_WHITE);
    // Set pixel color; 1 on the monochrome screen.
    ...
    {  
        cout << pstr("ok") << endl;
        cout << pstr("Begin with self-tests for MPU6050:") << endl;
        ...
        if(SelfTest[0] < 1.0f && SelfTest[1] < 1.0f && SelfTest[2] < 1.0f &&
         SelfTest[3] < 1.0f && SelfTest[4] < 1.0f && SelfTest[5] < 1.0f) {
            ...
            cout << pstr("MPU6050 initialized for active data mode, start calibration...");
...
void loop (void) {
    ...
    if(readByte(MPU6050_ADDRESS, INT_STATUS) & 0x01) {  // check if data ready interrupt
        ...
        //addToMovingAverageBuffer (ax_buf, ax);
        ay = (float)accelCount[1]*aRes - accelBias[1];
        //addToMovingAverageBuffer (ay_buf, ay);   
        az = (float)accelCount[2]*aRes - accelBias[2];  
        //addToMovingAverageBuffer (az_buf, az); 
    ...
    // Render text with 25Hz, thus, one frame stays for a period of 40ms.
    uint32_t current_time = millis() - time_of_last_frame;
    if(current_time > 40) {
      ...
      // compute magnitude of the accelerometer
      float magnitude = sqrt(sq(ax) + 
                             sq(ay) + 
                             sq(az));
...
// Set of useful function to access acceleration, gyroscope, and temperature
void getGres() {
  switch (Gscale)
  {
